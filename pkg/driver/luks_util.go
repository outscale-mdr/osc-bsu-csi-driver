...
func CheckLuksPassphrase(exec k8sExec.Interface, devicePath string, passphrase string) bool {
	passphraseCmd := exec.Command("cryptsetup", "-v", "--type=luks2", "--batch-mode", "--test-passphrase", "luksOpen", devicePath)
	passwordReader := strings.NewReader(passphrase)
	passphraseCmd.SetStdin(passwordReader)
	if _, err := passphraseCmd.CombinedOutput(); err != nil {
		return false
	}
...
func LuksOpen(exec Mounter, devicePath string, encryptedDeviceName string, passphrase string) (bool, error) {
	...
	passphraseCmd := exec.Command("cryptsetup", "-v", "--type=luks2", "--batch-mode", "luksOpen", devicePath, encryptedDeviceName)
	passwordReader := strings.NewReader(passphrase)
	passphraseCmd.SetStdin(passwordReader)
	if out, err := passphraseCmd.CombinedOutput(); err != nil {
